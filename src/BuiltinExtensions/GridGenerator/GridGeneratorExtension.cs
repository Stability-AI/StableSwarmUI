...
public class GridGeneratorExtension : Extension
{
    public override void OnInit()
    {
        ...
        {
            call.LocalData = new List<string>();
        };
        GridGenCore.GridCallParamAddHook = (call, param, val) =>
        {
            ...
            {
                (call.LocalData as List<string>).Add(val);
                return true;
            }
        ...
        {
            foreach (string replacement in call.LocalData as List<string>)
            {
                // TODO: apply_prompt_replace(param, replace);
            }
        };
    ...
    public async Task<JObject> GridGenRun(Session session, T2IParams baseInput, JObject raw, string outputFolderName, bool doOverwrite, bool fastSkip, bool generatePage, bool publishGenMetadata, bool dryRun)
    {
        ...
        try
        {
            GridGenCore.Run(baseInput, raw["gridAxes"], data, null, session.User.OutputDirectory, outputFolderName, doOverwrite, fastSkip, generatePage, publishGenMetadata, dryRun);
