...
function buildSettingsMenu(container, data, prefix, tracker) {
    ...
    function addBlock(block, blockPrefix = '') {
        ...
        for (let setting of groups) {
            let data = block[setting];
            let settingFull = `${blockPrefix}${setting}`;
            content += `<div class="input-group input-group-open settings-group" id="auto-group-${prefix}${settingFull}"><span id="input_group_${prefix}${settingFull}" class="input-group-header  group-label"><span class="header-label-wrap"><span class="auto-symbol">&#x2B9F;</span><span class="header-label">${translateableHtml(data.name)}: ${translateableHtml(escapeHtmlNoBr(data.description))}</span></span></span><div class="input-group-content" id="input_group_content_${prefix}${settingFull}">`;
...
function applyThemeSetting(theme_info) {
    setTimeout(() => {
        ...
        function setTheme() {
            ...
            header.innerHTML = theme.css_paths.map(path => `<link rel="stylesheet" href="${path}?${siteHeader.href.split('?')[1]}" />`).join('\n');
        }
